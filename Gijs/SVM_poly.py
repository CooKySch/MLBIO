...
def train_svm(x_train, y_train, y_valid, x_valid, model_name):
    ...
    for l in tqdm(lambdas):
        ...
        clf = MultiOutputRegressor(SVR(kernel='poly', C=l, gamma='auto'), n_jobs=-1).fit(x_train, y_train)
    ...
    for l in tqdm(lambdas):
        np.random.seed(0)
        # Initialize SVR model
        clf = MultiOutputRegressor(SVR(kernel='poly', C=l, gamma='scale'), n_jobs=-1).fit(x_train, y_train)
    ...
    np.random.seed(0)
    clf = MultiOutputRegressor(SVR(kernel='poly', C=l, gamma='auto'), n_jobs=-1).fit(x_valid, y_valid)

    pkl_file = f"Gijs/Data/SVR_{model_name}_poly_gamma_auto_reg.pkl"
    ...
    np.random.seed(0)
    clf = MultiOutputRegressor(SVR(kernel='poly', C=l, gamma='scale'), n_jobs=-1).fit(x_valid, y_valid)

    pkl_file = f"Gijs/Data/SVR_{model_name}_poly_gamma_scale_reg.pkl"
    with open(pkl_file, 'wb') as file:
        pkl.dump(clf, file)
